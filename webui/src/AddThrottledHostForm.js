...
class AddThrottledHostForm extends Component {
  state = {
    creating: false,
    name: '',
    hostname: '',
  ...
  async onSubmit() {
    var res = await fetch(`/api/throttle/add?hostname=${encodeURIComponent(this.state.hostname)}`);
    if (res.status === 401) {
...
export default AddThrottledHostForm;
